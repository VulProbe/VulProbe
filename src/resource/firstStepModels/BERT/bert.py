...
from bert_model import CodeBert
import pandas as pd
# metrics
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    # test_result_df.to_csv('test_result_44.csv', index=False)
    ...
    if args.do_explaination:
        tp_list = test_result_df[test_result_df.apply(lambda x: x['label'] == 1 and x['pred'] == True, axis=1)].index.tolist()
        fp_list = test_result_df[test_result_df.apply(lambda x: x['label'] == 0 and x['pred'] == True, axis=1)].index.tolist()
        tn_list = test_result_df[test_result_df.apply(lambda x: x['label'] == 0 and x['pred'] == False, axis=1)].index.tolist()
        fn_list = test_result_df[test_result_df.apply(lambda x: x['label'] == 1 and x['pred'] == False, axis=1)].index.tolist()
        print(f"tp: {len(tp_list)} | fp: {len(fp_list)} | tn: {len(tn_list)} | fn: {len(fn_list)}")
        
        methods = []
        if args.explaination_method == 'all':
            methods = ['deeplift_shap', 'gradient_shap', 'saliency']
        else:
        ...
        for reasoning_method in methods:
            ...
            for batch in progress_bar:
                ...
                if do_explaination:
                    if args.explaination_path_dir == '~':
                        raise ValueError("Please specify the path to save the explaination results.")
                    ...
                    if reasoning_method == 'attention':
                        ...
                        if index not in tn_list:          
                            ...
                            loss, logit, last_hidden_state, attentions = model(input_ids=input_ids, labels=labels, output_attentions=True)
                            ...
                            processed_word_att_scores = str([(item[0], float(item[1].detach().cpu().item())) for item in word_att_scores])
                        else:
                            lines_score = []
                            word_att_scores = []
                        ...
                        if not os.path.isfile(explaination_path):
                            ...
                            df = pd.DataFrame({'index':index, 'lines_score':[lines_score], 'word_att_scores':[processed_word_att_scores]})
                            df.to_csv(explaination_path, index=False)
                        else:
                            df = pd.read_csv(explaination_path)
                            new_data = pd.DataFrame({'index':index, 'lines_score':[lines_score], 'word_att_scores':[processed_word_att_scores]}, index=[0])
                        ...
                        if not os.path.isfile(explaination_path):
                            logger.info(f"Save Explaination to {explaination_path}")
                            df = pd.DataFrame({'index':index, 'lines_score':[all_lines_score], 'word_att_scores':[word_att_scores]})
                            df.to_csv(explaination_path, index=False)
                        else:
                            df = pd.read_csv(explaination_path)
                            new_data = pd.DataFrame({'index':index, 'lines_score':[all_lines_score], 'word_att_scores':[word_att_scores]}, index=[0])
                            df = df._append(new_data, ignore_index=True)
                            df.to_csv(explaination_path, index=False)
    ...
    if args.get_attention:
        saved_path = 'VulProbe/src/resource/firstStepModels/BERT/attention_score_map.csv'
        ...
        for batch in progress_bar:
            ...
            if do_get:
                ...
                if not os.path.isfile(saved_path):
                    ...
                    df = pd.DataFrame({"index": index, "attention_score_map": [to_df_attentions], "token_score_match": [token_score_match]})
                    df.to_csv(saved_path, index=False)
                else:
                    df = pd.read_csv(saved_path)
                    new_data = pd.DataFrame({"index": index, "attention_score_map": [to_df_attentions], "token_score_match": [token_score_match]})
                    df = df._append(new_data, ignore_index=True)
                    df.to_csv(saved_path, index=False)
...
def main():
    ...
    parser.add_argument('--explaination_path_dir', type=str, default='~',
                        help="The path to save the explaination results.")
