...
class ProgramArguments:
    do_train: bool = field(default=False, metadata={'help': 'Train the BiLSTM classifier.'})
    do_explain: bool = field(default=False, metadata={'help': 'Explain the BiLSTM classifier.'})
    explaination_method: str = field(default='deeplift', metadata={'help': 'Explain the BiLSTM classifier.'})
    save_path: str = field(default='codebert_bilstm_classifier.pth', metadata={'help': 'The path to save the BiLSTM classifier.'})
...
class BiLSTM_Config:
    ...
    use_cuda = USE_CUDA
    save_path = 'codebert_bilstm_classifier.pth'
    sampleRate = 2
    labelSelected = 2 
...
def train_model(config, data_train):
    model = codebert_lstm(config.hidden_dim,
                    config.output_size,
                    config.n_layers,
...
def test_model(config, data_test):
    ...
    if args.do_explain:
        ...
        test_dataset = pd.read_json('/ssd2/wqq/work4/LineProbe/src/resource/dataset/c/test.jsonl', orient="records", lines=True)
            ...
            for input_ids, labels in tqdm(data_test):
                ...
                if not do_explaination:
                    ...
                    if reasoning_method == "deeplift" or \
                        ...
                        def lig_forward(input_embeds):
                            pred_prob = model(input_embeds.float())[0]
                            # print(type(input_ids))
                            # pred_prob = model(input_ids)[0]
                            return pred_prob
                        ...
                        lig = LayerIntegratedGradients(lig_forward, model)
                        
                        input_embeds = model.codebert(inputs, attention_mask=inputs.ne(1))[0].float()
                        baselines = torch.zeros(1, 200, 768, requires_grad=True).to(args.device)
                        attributions, delta = lig.attribute(inputs=input_embeds,
                                                            baselines=baselines,
...
def myDataProcess(dataFile):
    ...
    logger.info(f"load data (length: {len(df)}) successfully!")
...
if __name__ == '__main__':
    ...
    if args.do_train:
        ...
        logger.info(f"训练轮数:{model_config.epochs}，总耗时:{total_time}s")
